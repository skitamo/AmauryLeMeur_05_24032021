let request = new XMLHttpRequest();
	request.onreadystatechange = function () {
		if (this.readyState == XMLHttpRequest.DONE && this.status == 200) {
			...
			mainCart.appendChild(cartFinalPriceContainer);
			cartFinalPriceContainer.appendChild(cartFinalPrice);
			...
			cartName.innerHTML = response.name;
			cartPrice.innerHTML = response.price / 100 + ",00 €";
			cartID.innerHTML = response._id;
			cartQuantity.innerHTML = cartUnit.quantity;
	...
	request.open("GET", "http://localhost:3000/api/teddies/" + cartUnitID.id);
	request.send();
